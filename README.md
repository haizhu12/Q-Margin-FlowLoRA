...
The following is an actual output of `scripts/run_background_replace.py`, not an experiment plot. A synthetic white-background bicycle generated from a text-only prompt is used as the input; the local frozen FLUX.2 model generates an empty forest background, and the demo composites the bicycle without asking the diffusion model to redraw it. Asset hashes, the source prompt, and generation provenance are recorded in [`assets/readme/PROVENANCE.md`](assets/readme/PROVENANCE.md).

<table>
  <tr>
    <th width="50%">Input</th>
    <th width="50%">Final edit</th>
  </tr>
  <tr>
    <td><img src="assets/readme/bicycle_input.png" alt="Unbranded red mountain bicycle on a white background"></td>
    <td><img src="assets/readme/bicycle_edited_result.png" alt="The same red bicycle composited onto the generated forest trail"></td>
  </tr>
</table>

<details>
  <summary>Show the empty background generated by FLUX.2</summary>
  <p align="center"><img src="assets/readme/bicycle_generated_background.png" width="512" alt="Empty forest trail generated by FLUX.2"></p>
</details>

For this run, 26,855 fully opaque foreground pixels were verified byte-for-byte against the input (`max_abs_foreground_error = 0`). Soft boundary pixels are decontaminated and blended with the generated background. This guarantee applies only to the locked core of the compositing demo; it is not a general guarantee for diffusion-based full-image editing.

The exact command used to reproduce this example is in [Run the bicycle demo](#run-the-bicycle-demo). This deployment demo is separate from the paper's five-route Q-Margin quantitative protocol.
